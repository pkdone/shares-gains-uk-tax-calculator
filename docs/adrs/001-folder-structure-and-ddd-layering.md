...
| Persistence | `src/infrastructure/persistence/schemas/` | `.extend()` domain bases (e.g. `shareAcquisitionBaseSchema`, `holdingCreateSchema`) with tenancy and timestamps; keep string ids in Zod for `$jsonSchema`; map BSON `ObjectId` in repository code only |
| Transport | `src/app/` (co-located with route handlers) | `.pick()`, `.omit()`, `.extend()` for request/response and form payloads (e.g. `stockSymbolSchema` for create-holding) |
