...
- **`manual_gbp`:** Milestone 2 behaviour — `grossConsiderationGbp`, `feesGbp`. Used for hand-entered acquisitions.
- **`import_usd`:** Vest imports — `grossConsiderationUsd`, `feesUsd`. **No GBP fields** on the document until Milestone 5 derives sterling per transaction.
